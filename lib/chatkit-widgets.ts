...
// Create a cheat sheet preview widget
...
export function createCheatSheetWidget(sheet: CheatSheetWidget): any {
  return {
    type: 'Card',
    size: 'lg',
    padding: { x: 16, y: 12 },
    ...
    children: [
      ...
      sheet.pickCount && sheet.avgConfidence ? {
        type: 'Row',
        gap: 16,
        children: [
          {
      ...
      } : null,
      {
        ...
        children: [
          {
            type: 'Markdown',
            value: `🔗 [View & Download Cheat Sheet](${sheet.shareUrl})`
          }
        ]
      }
    ].filter(Boolean),
    status: {
      text: '✨ Cheat Sheet Ready',
      icon: 'sparkle'
    }
  }
}
